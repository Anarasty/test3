...
async function getMovies() {
    ...
    const res = await fetch(`https://www.omdbapi.com/?apikey=4722b43c&s=${searchValue}`).catch(handleError);
...
Movieresults.addEventListener("click", function (event) {
    ...
    if (event.target.classList.contains("favoriteAddButton")) {
        btnsArr[btnIndex].classList.remove("favoriteAddButton");
        btnsArr[btnIndex].classList.add("remove-watchlist-btn");

        // Add selected movie to watchlistMovies array
        favoriteMovies.push(moviesArr[btnIndex]);
    } else if (event.target.classList.contains("remove-watchlist-btn")) {
        btnsArr[btnIndex].classList.remove("remove-watchlist-btn");
        btnsArr[btnIndex].classList.add("favoriteAddButton");

        // Get index of the movie want to remove
        const removedIndex = favoriteMovies.indexOf(moviesArr[btnIndex]);
        // Remove movie from watchlistMovies array
        favoriteMovies.splice(removedIndex, 1);
    }
